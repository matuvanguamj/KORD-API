...
from reportlab.platypus import (
    SimpleDocTemplate, Paragraph, Spacer, Table as _RLTable, TableStyle,
...
def Table(data, colWidths=None, **kwargs):
    """Safe Table wrapper — skips empty tables."""
    if not data:
        return Spacer(1, 1)
    clean = []
    for row in data:
    ...
    max_cols = max((len(r) for r in clean if isinstance(r, (list,tuple))), default=0)
    if max_cols == 0:
        return Spacer(1, 1)
    if colWidths and len(colWidths) != max_cols:
        colWidths = colWidths[:max_cols] if len(colWidths) > max_cols else colWidths + [colWidths[-1]] * (max_cols - len(colWidths))
    try:
        return _RLTable(clean, colWidths=colWidths, **kwargs)
...
def generate_prereport_pdf(consolidated, recommendations, all_results,
    ...
    if chart_row and len(chart_row) > 0:
        try:
            n = len(chart_row)
            if n >= 2:
                story.append(Table([chart_row[:2]], colWidths=[155*mm, MW-155*mm],
                ...
                story.append(Table([[chart_row[0]]], colWidths=[MW],
                    style=TableStyle([("ALIGN",(0,0),(-1,-1),"CENTER"),
                                      ("BACKGROUND",(0,0),(-1,-1),OFF),
                                      ("TOPPADDING",(0,0),(-1,-1),6),
                                      ("BOTTOMPADDING",(0,0),(-1,-1),6)])))
        except Exception as _e:
            print(f"Chart table error: {_e}")
    ...
    cells = []
    analyse_piliers = analyse_piliers or {}
    if not analyse_piliers:
        # Fallback: use consolidated data directly
        for key in pilier_order:
            ...
            if isinstance(a, dict):
                analyse_piliers[key] = {
                    ...
                    "max": a.get("max",20),
                    "niveau": "moyen",
                    "analyse": f"Score : {a.get('score',0)}/{a.get('max',20)} pts.",
                    "chiffres_cles": [],
                    "risque_principal": ""
                }
    for key in pilier_order:
    ...
    for i in range(0, len(cells), 2):
        c1 = cells[i]
        c2 = cells[i+1] if i+1 < len(cells) else Spacer(col_w, 10)
        if c1 is not None:
            rows_2.append([c1, c2])
    if rows_2:
        # Filter out any rows with empty cells
        clean_rows = [r for r in rows_2 if len(r) == 2 and r[0] is not None]
        if clean_rows:
            try:
                story.append(Table(clean_rows, colWidths=[col_w, col_w], style=TableStyle([
                    ("LEFTPADDING",(0,0),(-1,-1),0),("RIGHTPADDING",(0,0),(-1,-1),0),
                    ("TOPPADDING",(0,0),(-1,-1),0),("BOTTOMPADDING",(0,0),(-1,-1),4),
                    ("VALIGN",(0,0),(-1,-1),"TOP"),
                ])))
            except Exception as _te:
                print(f"Pilier table error: {_te}")
