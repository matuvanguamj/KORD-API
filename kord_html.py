...
def generate_prereport_html(
    ...
    dormance_png=None, breakdown_png=None, cash_png=None,
) -> str:
    ...
    dormance_b64 = img_to_b64(dormance_png)
    breakdown_b64= img_to_b64(breakdown_png)
    cash_b64     = img_to_b64(cash_png)
    ...
    # Compute pilier rows for cover page
    _cp_rows = ""
    for _k in pilier_order:
        _lbl = pilier_meta[_k][0].split(" et ")[0]
        _mx  = pilier_meta[_k][1]
        _sc  = (consolidated.get("analyses") or {}).get(_k, {}).get("score", 0)
        _pct = round(_sc/_mx*100) if _mx > 0 else 0
        _cp_rows += f'<div class="cover-pilier-row"><span class="cover-pilier-name">{_lbl}</span><div class="cover-pilier-bar-wrap"><div class="cover-pilier-bar" style="width:{_pct}%"></div></div><span class="cover-pilier-score">{_sc}/{_mx}</span></div>'
    cover_pilier_rows = _cp_rows
    ...
    dormance_img_tag = f'<img src="{dormance_b64}" class="chart-full" alt="Dormance">' if dormance_b64 else ""
    breakdown_img_tag= f'<img src="{breakdown_b64}" class="chart-full" alt="Breakdown">' if breakdown_b64 else ""
    cash_img_tag     = f'<img src="{cash_b64}" class="chart-full" alt="Cash Impact">' if cash_b64 else "" 
...
<div class="page">
  ...
  <div class="section-header"><span class="section-num">02</span><span class="section-title">Analyse Graphique des Données Client</span></div>
  <p class="section-sub">Visualisations basées sur les données réelles — Impact financier estimé</p>

  {breakdown_img_tag}
  <p class="chart-source">Décomposition du score — Les points perdus correspondent à de l'argent récupérable</p>

  {cash_img_tag}
  <p class="chart-source">Potentiel de gain estimé par pilier — Basé sur les points non obtenus</p>

  {dormance_img_tag}
  <p class="chart-source">Références en dormance par segment — Cash immobilisé dans vos stocks</p>

  <div class="charts-row">
    {bar_img_tag}
    {radar_img_tag}
  </div>
  <p class="chart-source">Score KORD vs Benchmark marché PME — Source : Moteur d'analyse KORD</p>
...
document.addEventListener('DOMContentLoaded', function() {{
  var sels = 'p,h1,h2,h3,.body-text,.pilier-analyse,.chiffre-cle,.risque-val,.pull-quote p,.bench-val,.stat-num,.stat-label,.prio-field p,.impact-big,.gain-val,.opp-card p,.vig-item p,.q-item p,.next-step-box p,.section-sub,.prio-titre,.pilier-titre,.cover-company,.cover-client,.cover-meta,.score-big,td,.cr-impact,.opp-gain';
  document.querySelectorAll(sels).forEach(function(el) {{
    el.setAttribute('contenteditable','true');
    el.setAttribute('spellcheck','false');
    el.style.outline='none';
    el.style.cursor='text';
    el.addEventListener('focus',function(){{this.style.background='rgba(0,0,0,0.04)';}});
    el.addEventListener('blur',function(){{this.style.background='';}});
  }});
  var tb=document.createElement('div');
  tb.style.cssText='position:fixed;top:12px;right:12px;z-index:9999;display:flex;gap:10px;background:#000;padding:8px 16px;font-family:Space Grotesk,sans-serif;font-size:10px;color:#fff;align-items:center;box-shadow:0 2px 20px rgba(0,0,0,0.3)';
  tb.innerHTML='<span style="color:#666;letter-spacing:.15em">KORD ÉDITEUR</span><span style="color:#333">|</span><span style="color:#999">Cliquez sur un texte pour le modifier</span><span style="color:#333">|</span><button onclick="window.print()" style="background:#fff;color:#000;border:none;padding:5px 14px;font-family:Space Grotesk,sans-serif;font-size:10px;font-weight:700;cursor:pointer;letter-spacing:.08em">EXPORTER PDF →</button>';
