...
from kord_charts import generate_radar_chart, generate_bar_chart, generate_gauge_chart, generate_dormance_chart, generate_score_breakdown_chart, generate_cash_impact_chart
...
async def analyze_global(req: AnalyzeGlobalRequest):
    ...
    html_url = None
    try:
        html_content = generate_prereport_html(
        ...
        sb.storage.from_("client-files").upload(html_path, html_bytes, {"content-type":"text/html; charset=utf-8"})
        signed3 = sb.storage.from_("client-files").create_signed_url(html_path, 60*60*24*30)
        html_url = (signed3.get("signedURL") or signed3.get("signedUrl") or
                   (signed3.get("data") or {}).get("signedURL"))
        print(f"HTML généré")
    except Exception as e:
